<div align="center">
  <h1>
    <samp>Hi there! I'm Gabriel 🌱</samp>
  </h1>
  <img src="https://raw.githubusercontent.com/Luveira/Luveira/main/Images/Presenting_NoBG.gif" width="550" alt="Tree">
</div>

<div align="center">
  <!-- Cabeçalho de Seleção de Idioma -->
  <h3><samp>🔤 Escolha seu Idioma | Choose Your Language</samp></h3>
  <a href="#portugues">
    <img src="https://img.shields.io/badge/BR-PORTUGUÊS-333333?style=for-the-badge" alt="Português">
  </a>
  <a href="#english">
    <img src="https://img.shields.io/badge/EN-ENGLISH-333333?style=for-the-badge" alt="English">
  </a>
</div>

<br><br>
<hr>
<br>

<!-- SEÇÃO EM PORTUGUÊS -->
<div id="portugues" align="left">
  <h2><samp>🇧🇷 Português</samp></h2>
  <p>
    <samp>
      Engenheiro de Materiais de formação, tenho mestrado em Desenvolvimento Regional Sustentável pela Universidade Federal do Cariri (UFCA). Durante a pesquisa de dissertação:
    </samp>
  </p>
  
  <samp>
    ✅ Modelei Séries Temporais usando <em>Discrete Bayesian Networks</em> (DBN), ARIMA, SVR, GRU e ETS (Versão em Espaço de Estado do <em>Exponential Smoothing</em>)<br>
    ✅ Apliquei o paradigma de Aprendizado Online para Séries com <em>Concept Drift</em> e técnicas de <em>Ensemble Learning</em><br>
    ✅ Trabalhei com testes de hipótese para comparação de previsões de Séries Temporais: Diebold-Mariano
  </samp>

  <p>
    <samp>
      <br>
      Atualmente, estou trabalhando na construção do meu portifólio. Sempre buscando diversificar o meu conhecimento para além da academia e buscando levar rigor acadêmico aos cases aqui presentes 🧪👨🏻‍💻<br>
      Portifólio em construção 🚧
    </samp>
  </p>
</div>

<br><br>
<hr>
<br>

<!-- SEÇÃO EM INGLÊS -->
<div id="english" align="left">
  <h2><samp>🇺🇸 English</samp></h2>
  <p>
    <samp>
      Graduated in Materials Engineering, I earned my Master's in Regional Sustainable Development from the Federal University of Cariri (UFCA). During my dissertation research:
    </samp>
  </p>

  <samp>
    ✅ I modeled Time Series using <em>Discrete Bayesian Networks</em> (DBN), ARIMA, SVR, GRU, and ETS (State Space Version of <em>Exponential Smoothing</em>)<br>
    ✅ I applied the Online Learning paradigm for Series with <em>Concept Drift</em> and <em>Ensemble Learning</em> techniques<br>
    ✅ I worked with hypothesis testing for Time Series forecast comparison: Diebold-Mariano
  </samp>

  <p>
    <samp>
      <br>
      Currently, I am working on building my portfolio. Always seeking to diversify my knowledge beyond academia and aiming to bring academic rigor to the cases presented here 🧪👨🏻‍💻
      <br>
      Portfolio under construction 🚧
    </samp>
  </p>
</div>
